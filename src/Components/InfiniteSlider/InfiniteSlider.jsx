    import React, { useEffect, useRef, useState } from 'react';
import './InfinitySlider.css'

function InfinitySlider() {

    const playSVG = (<svg className="ml-[5.5px] mt-[5.5px]" height="10px" width="12px" viewBox="0 0 512 512"><path d="M405.2,232.9L126.8,67.2c-3.4-2-6.9-3.2-10.9-3.2c-10.9,0-19.8,9-19.8,20H96v344h0.1c0,11,8.9,20,19.8,20  c4.1,0,7.5-1.4,11.2-3.4l278.1-165.5c6.6-5.5,10.8-13.8,10.8-23.1C416,246.7,411.8,238.5,405.2,232.9z"/></svg>)
    const pauseSVG = (<svg className="ml-[4.5px] mt-[5.5px]" height="10px" width="12px" viewBox="0 0 512 512"><g><path d="M224,435.8V76.1c0-6.7-5.4-12.1-12.2-12.1h-71.6c-6.8,0-12.2,5.4-12.2,12.1v359.7c0,6.7,5.4,12.2,12.2,12.2h71.6   C218.6,448,224,442.6,224,435.8z"/><path d="M371.8,64h-71.6c-6.7,0-12.2,5.4-12.2,12.1v359.7c0,6.7,5.4,12.2,12.2,12.2h71.6c6.7,0,12.2-5.4,12.2-12.2V76.1   C384,69.4,378.6,64,371.8,64z"/></g></svg>)
    
    const [stop , setStop] = useState(false);
    const wrapperRef = useRef(null);

    useEffect(() => {
        const wrapper = wrapperRef.current;
        const covers = wrapper.querySelectorAll('.cover');
    
        covers.forEach((cover) => {
          const shopNow = cover.querySelector('.btn');

          const handleMouseEnter = () => {
            cover.style.opacity = '100%';
            shopNow.style.transform = 'translateY(0px)';
            // wrapper.style.animationPlayState = 'paused';
            setStop(true)
          };
    
          const handleMouseLeave = () => {
            cover.style.opacity = '0';
            shopNow.style.transform = 'translateY(25px)';
            // wrapper.style.animationPlayState = 'running';
            setStop(false)
          };
    
          cover.addEventListener('mouseenter', handleMouseEnter);
          cover.addEventListener('mouseleave', handleMouseLeave);
    
          return () => {
            cover.removeEventListener('mouseenter', handleMouseEnter);
            cover.removeEventListener('mouseleave', handleMouseLeave);
          };
        });
      }, [wrapperRef]);

      useEffect(()=>{
        const wrapper = wrapperRef.current;

        if(stop===true){
            wrapper.style.animationPlayState = 'paused';
        }
        else{
            wrapper.style.animationPlayState = 'running';
        }
      },[stop])

      const togglePlayPause = () => {
        setStop(!stop);
    };

    return (
    <>
            <br /><br /><br />
            
        <div className='relative'>

            <div className='overflow-hidden slider'>
                <div className='flex w-max h-[188px] lg:h-[200px] xl:h-[210px] 2xl:h-[230px] wrapper' ref={wrapperRef}>
                          
                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features126/v4/70/f8/53/70f85351-a46f-39aa-724d-691d23df6893/d32ee8a7-c03d-4459-82aa-5b7cf6dee412.png/834x472.jpg" />
                                    
                                    <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 387 80"><path d="M117.81,58.88H89.96l-6.68,19.75h-11.8L97.86,5.57h12.25l26.38,73.06h-12L117.81,58.88z M92.85,49.77h22.07l-10.89-32.05 h-0.3L92.85,49.77z M142.71,25.36h10.43v9.06h0.25c1.77-6.23,6.53-9.92,12.81-9.92c1.57,0,2.94,0.25,3.75,0.41v10.18 c-0.86-0.35-2.78-0.61-4.91-0.61c-7.04,0-11.39,4.71-11.39,12.25v31.9h-10.94V25.36z M209.95,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L209.95,43.69L209.95,43.69z M226.11,63.49c0-9.32,7.14-15.04,19.8-15.75l14.58-0.86v-4.1 c0-5.92-4-9.47-10.68-9.47c-6.33,0-10.28,3.04-11.24,7.8h-10.33c0.61-9.62,8.81-16.71,21.97-16.71c12.91,0,21.16,6.83,21.16,17.52 v36.71h-10.48v-8.76h-0.25c-3.09,5.92-9.82,9.67-16.81,9.67C233.4,79.54,226.11,73.06,226.11,63.49z M260.49,58.68v-4.2l-13.11,0.81 c-6.53,0.46-10.23,3.34-10.23,7.9c0,4.66,3.85,7.7,9.72,7.7C254.51,70.88,260.49,65.62,260.49,58.68z M279.06,52 c0-16.81,8.91-27.49,22.28-27.49c7.64,0,13.72,3.85,16.71,9.72h0.2V5.57h10.99v73.06H318.6v-9.06h-0.2 c-3.14,6.07-9.21,9.97-16.96,9.97C287.97,79.54,279.06,68.8,279.06,52z M290.25,52c0,11.24,5.42,18.28,14.08,18.28 c8.4,0,14.02-7.14,14.02-18.28c0-11.04-5.62-18.28-14.02-18.28C295.67,33.72,290.25,40.81,290.25,52z M385.16,62.98 c-1.52,9.72-10.73,16.66-22.93,16.66c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58 v3.75h-37.62v0.66c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.83-3.14,12.3-7.8C374.78,62.98,385.16,62.98,385.16,62.98z M348.2,47.08 h26.63c-0.25-8.15-5.42-13.87-13.06-13.87C354.13,33.21,348.76,39.03,348.2,47.08z M43.87,12.7c2.73-3.42,4.59-8.01,4.1-12.7 c-4,0.2-8.88,2.64-11.71,6.06c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.92,19.14 c-6.52-0.39-12.07,3.7-15.18,3.7c-3.12,0-7.89-3.51-13.04-3.41c-6.71,0.1-12.94,3.89-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83 c3.31,4.87,7.3,10.23,12.55,10.03c4.97-0.19,6.91-3.22,12.95-3.22c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74 c3.8-5.55,5.35-10.91,5.45-11.2c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1 C57.75,20.12,50.45,19.34,47.92,19.14"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>


                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative bg-blue-950'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-[55%] h-[90%] pt-5 hidden lg:inline absolute left-4 object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/e9/fd/65/e9fd6546-8620-4445-24bb-feeb6a25e42c/0c233023-fb62-4129-b375-7b9278bb2d78.png/392x392SC.DN01.jpg?l=en-US" />
                                    <img className='w-[45%] md:w-[44%] lg:w-[55%] h-[80%] sm:h-[85%] md:h-[90%] pt-[23px] md:pt-3 lg:hidden absolute left-[28%] object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/e9/fd/65/e9fd6546-8620-4445-24bb-feeb6a25e42c/0c233023-fb62-4129-b375-7b9278bb2d78.png/196x196SC.DN01.jpg?l=en-US" />
                                    
                                    <svg className='w-[65px] lg:w-[73px] h-[12px] lg:h-[16px] absolute bottom-4 right-1 md:right-2 lg:right-3' fill='white' viewBox="0 0 339 80"><path d="M142.14,78.63V24.86h-0.35l-22.02,53.77h-8.4L89.28,24.86h-0.35v53.77H78.6V5.57h13.11l23.64,58.22h0.41l23.59-58.22h13.16 v73.06H142.14z M209.62,78.63h-10.48v-9.01h-0.25c-2.94,6.38-8.35,9.97-16.5,9.97c-11.59,0-18.68-7.54-18.68-19.75V25.36h10.94 v32.35c0,8.1,3.85,12.45,11.39,12.45c7.9,0,12.66-5.47,12.66-13.77V25.36h10.94L209.62,78.63z M239.59,24.4 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.84 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C219.03,30.93,227.34,24.4,239.59,24.4L239.59,24.4z M269.26,11.34 c-0.07-3.51,2.72-6.41,6.23-6.48s6.41,2.72,6.48,6.23s-2.72,6.41-6.23,6.48c-0.05,0-0.1,0-0.15,0 C272.12,17.59,269.29,14.81,269.26,11.34L269.26,11.34z M270.12,25.36h10.94v53.26h-10.94V25.36z M327.22,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L327.22,43.69z M43.87,12.7c2.96-3.54,4.43-8.09,4.1-12.7c-4.54,0.43-8.74,2.6-11.71,6.06 c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.91,19.14c-6.52-0.39-12.07,3.7-15.18,3.7 c-3.12,0-7.89-3.51-13.04-3.41c-6.83,0.17-13.05,3.95-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83c3.31,4.87,7.3,10.23,12.55,10.03 C25.83,79.03,27.77,76,33.81,76c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74c2.35-3.45,4.18-7.22,5.45-11.2 c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1C57.75,20.12,50.45,19.34,47.91,19.14"></path></svg>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>


                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Video221/v4/b1/cd/c7/b1cdc76a-0d1d-fcc2-7e04-ecd73c11442b/media-ST_KN_0056-8864373-Artwork-iOS-283786.png/834x472.jpg" />
                                    
                                   <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 449 80"><path d="M78.6,78.56V5.5h45.62v9.77H89.94v23.14h31.39v9.57H89.94v30.58H78.6z M131.86,11.22c0-3.49,2.83-6.28,6.33-6.28 c3.49,0,6.33,2.79,6.33,6.28c0,3.44-2.83,6.28-6.33,6.28C134.69,17.5,131.86,14.67,131.86,11.22z M132.72,25.3h10.94v53.26h-10.94 V25.3z M170.68,12.69V25.3h10.12v8.66h-10.12v29.36c0,4.61,2.02,6.68,6.48,6.68c1.11,0,2.89-0.1,3.59-0.25v8.66 c-1.21,0.3-3.64,0.51-6.08,0.51c-10.78,0-14.99-4.1-14.99-14.38V33.96h-7.75V25.3h7.75V12.69H170.68z M189.98,25.3h10.43v8.91h0.25 c2.89-6.08,8.56-9.82,16.76-9.82c11.75,0,18.48,7.49,18.48,19.69v34.48h-10.99V46.21c0-8-3.75-12.4-11.34-12.4 c-7.7,0-12.66,5.47-12.66,13.72v31.04h-10.94V25.3z M291.07,62.91c-1.52,9.72-10.73,16.66-22.93,16.66 c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58v3.75h-37.62v0.66 c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.84-3.14,12.3-7.8H291.07z M254.11,47.02h26.63c-0.25-8.15-5.42-13.87-13.06-13.87 C260.03,33.15,254.66,38.97,254.11,47.02z M319.62,24.34c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4 c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4 c-13.32,0-21.32-6.53-22.33-16.66h10.83c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79 l-8.66-2.02c-10.13-2.33-15.14-7.19-15.14-14.94C299.07,30.87,307.37,24.34,319.62,24.34z M369.08,24.34 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.83 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C348.52,30.87,356.83,24.34,369.08,24.34z M428.07,78.63h-9.97V56.93h-20.66v-9.87h20.66V25.37 h9.97v21.69h20.71v9.87h-20.71V78.63z M43.87,12.64C46.64,9.29,48.5,4.63,47.99,0c-3.98,0.16-8.8,2.65-11.66,6 c-2.56,2.96-4.81,7.71-4.2,12.25C36.57,18.59,41.11,15.99,43.87,12.64z M47.89,19.13c-6.52-0.48-12.05,3.64-15.13,3.64 c-3.13,0-7.96-3.54-13.07-3.44c-6.72,0.1-12.91,3.91-16.38,9.93c-6.98,12.11-1.78,30.06,5.02,39.89c3.33,4.8,7.29,10.22,12.5,10.02 c5.01-0.2,6.91-3.24,12.97-3.24c6.06,0,7.77,3.24,13.07,3.15c5.4-0.1,8.82-4.9,12.12-9.73c3.82-5.57,5.39-10.97,5.48-11.26 c-0.12-0.05-10.52-4.04-10.62-16.02c-0.09-10.02,8.18-14.83,8.55-15.07C57.7,20.08,50.45,19.32,47.89,19.13z"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>

                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features116/v4/1c/d7/e1/1cd7e1f9-5c0d-0f83-0900-f9fcad401c05/71e4f76e-d72a-40f5-ad22-382f85f85065.png/834x472.jpg" />
                                    
                                   <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 387 80"><path d="M117.81,58.88H89.96l-6.68,19.75h-11.8L97.86,5.57h12.25l26.38,73.06h-12L117.81,58.88z M92.85,49.77h22.07l-10.89-32.05 h-0.3L92.85,49.77z M142.71,25.36h10.43v9.06h0.25c1.77-6.23,6.53-9.92,12.81-9.92c1.57,0,2.94,0.25,3.75,0.41v10.18 c-0.86-0.35-2.78-0.61-4.91-0.61c-7.04,0-11.39,4.71-11.39,12.25v31.9h-10.94V25.36z M209.95,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L209.95,43.69L209.95,43.69z M226.11,63.49c0-9.32,7.14-15.04,19.8-15.75l14.58-0.86v-4.1 c0-5.92-4-9.47-10.68-9.47c-6.33,0-10.28,3.04-11.24,7.8h-10.33c0.61-9.62,8.81-16.71,21.97-16.71c12.91,0,21.16,6.83,21.16,17.52 v36.71h-10.48v-8.76h-0.25c-3.09,5.92-9.82,9.67-16.81,9.67C233.4,79.54,226.11,73.06,226.11,63.49z M260.49,58.68v-4.2l-13.11,0.81 c-6.53,0.46-10.23,3.34-10.23,7.9c0,4.66,3.85,7.7,9.72,7.7C254.51,70.88,260.49,65.62,260.49,58.68z M279.06,52 c0-16.81,8.91-27.49,22.28-27.49c7.64,0,13.72,3.85,16.71,9.72h0.2V5.57h10.99v73.06H318.6v-9.06h-0.2 c-3.14,6.07-9.21,9.97-16.96,9.97C287.97,79.54,279.06,68.8,279.06,52z M290.25,52c0,11.24,5.42,18.28,14.08,18.28 c8.4,0,14.02-7.14,14.02-18.28c0-11.04-5.62-18.28-14.02-18.28C295.67,33.72,290.25,40.81,290.25,52z M385.16,62.98 c-1.52,9.72-10.73,16.66-22.93,16.66c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58 v3.75h-37.62v0.66c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.83-3.14,12.3-7.8C374.78,62.98,385.16,62.98,385.16,62.98z M348.2,47.08 h26.63c-0.25-8.15-5.42-13.87-13.06-13.87C354.13,33.21,348.76,39.03,348.2,47.08z M43.87,12.7c2.73-3.42,4.59-8.01,4.1-12.7 c-4,0.2-8.88,2.64-11.71,6.06c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.92,19.14 c-6.52-0.39-12.07,3.7-15.18,3.7c-3.12,0-7.89-3.51-13.04-3.41c-6.71,0.1-12.94,3.89-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83 c3.31,4.87,7.3,10.23,12.55,10.03c4.97-0.19,6.91-3.22,12.95-3.22c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74 c3.8-5.55,5.35-10.91,5.45-11.2c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1 C57.75,20.12,50.45,19.34,47.92,19.14"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>

                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative bg-orange-100'>

                                    <img className='w-[55%] h-[90%] pt-5 hidden lg:inline absolute left-4 object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/d9/e4/d2/d9e4d2c4-471e-1d0b-3afa-06fafa91a5c7/afd93571-31f8-49a5-a094-ea42fec182c9.png/392x392SC.DN01.jpg?l=en-US" />
                                    <img className='w-[45%] md:w-[44%] lg:w-[55%] h-[80%] sm:h-[85%] md:h-[90%] pt-[23px] md:pt-3 lg:hidden absolute left-[28%] object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/d9/e4/d2/d9e4d2c4-471e-1d0b-3afa-06fafa91a5c7/afd93571-31f8-49a5-a094-ea42fec182c9.png/262x262SC.DN01.jpg?l=en-US" />
                                    
                                    <svg className='w-[65px] lg:w-[73px] h-[12px] lg:h-[16px] absolute bottom-4 right-1 md:right-2 lg:right-3' fill='black' viewBox="0 0 339 80"><path d="M142.14,78.63V24.86h-0.35l-22.02,53.77h-8.4L89.28,24.86h-0.35v53.77H78.6V5.57h13.11l23.64,58.22h0.41l23.59-58.22h13.16 v73.06H142.14z M209.62,78.63h-10.48v-9.01h-0.25c-2.94,6.38-8.35,9.97-16.5,9.97c-11.59,0-18.68-7.54-18.68-19.75V25.36h10.94 v32.35c0,8.1,3.85,12.45,11.39,12.45c7.9,0,12.66-5.47,12.66-13.77V25.36h10.94L209.62,78.63z M239.59,24.4 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.84 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C219.03,30.93,227.34,24.4,239.59,24.4L239.59,24.4z M269.26,11.34 c-0.07-3.51,2.72-6.41,6.23-6.48s6.41,2.72,6.48,6.23s-2.72,6.41-6.23,6.48c-0.05,0-0.1,0-0.15,0 C272.12,17.59,269.29,14.81,269.26,11.34L269.26,11.34z M270.12,25.36h10.94v53.26h-10.94V25.36z M327.22,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L327.22,43.69z M43.87,12.7c2.96-3.54,4.43-8.09,4.1-12.7c-4.54,0.43-8.74,2.6-11.71,6.06 c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.91,19.14c-6.52-0.39-12.07,3.7-15.18,3.7 c-3.12,0-7.89-3.51-13.04-3.41c-6.83,0.17-13.05,3.95-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83c3.31,4.87,7.3,10.23,12.55,10.03 C25.83,79.03,27.77,76,33.81,76c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74c2.35-3.45,4.18-7.22,5.45-11.2 c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1C57.75,20.12,50.45,19.34,47.91,19.14"></path></svg>

                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>

                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features116/v4/03/e7/9c/03e79ced-818d-3cf2-995d-b8a86fa70492/118e58b2-3c86-4012-b8ef-9ba05e3f338a.png/572x322.jpg" />
                                    
                                    <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 449 80"><path d="M78.6,78.56V5.5h45.62v9.77H89.94v23.14h31.39v9.57H89.94v30.58H78.6z M131.86,11.22c0-3.49,2.83-6.28,6.33-6.28 c3.49,0,6.33,2.79,6.33,6.28c0,3.44-2.83,6.28-6.33,6.28C134.69,17.5,131.86,14.67,131.86,11.22z M132.72,25.3h10.94v53.26h-10.94 V25.3z M170.68,12.69V25.3h10.12v8.66h-10.12v29.36c0,4.61,2.02,6.68,6.48,6.68c1.11,0,2.89-0.1,3.59-0.25v8.66 c-1.21,0.3-3.64,0.51-6.08,0.51c-10.78,0-14.99-4.1-14.99-14.38V33.96h-7.75V25.3h7.75V12.69H170.68z M189.98,25.3h10.43v8.91h0.25 c2.89-6.08,8.56-9.82,16.76-9.82c11.75,0,18.48,7.49,18.48,19.69v34.48h-10.99V46.21c0-8-3.75-12.4-11.34-12.4 c-7.7,0-12.66,5.47-12.66,13.72v31.04h-10.94V25.3z M291.07,62.91c-1.52,9.72-10.73,16.66-22.93,16.66 c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58v3.75h-37.62v0.66 c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.84-3.14,12.3-7.8H291.07z M254.11,47.02h26.63c-0.25-8.15-5.42-13.87-13.06-13.87 C260.03,33.15,254.66,38.97,254.11,47.02z M319.62,24.34c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4 c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4 c-13.32,0-21.32-6.53-22.33-16.66h10.83c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79 l-8.66-2.02c-10.13-2.33-15.14-7.19-15.14-14.94C299.07,30.87,307.37,24.34,319.62,24.34z M369.08,24.34 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.83 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C348.52,30.87,356.83,24.34,369.08,24.34z M428.07,78.63h-9.97V56.93h-20.66v-9.87h20.66V25.37 h9.97v21.69h20.71v9.87h-20.71V78.63z M43.87,12.64C46.64,9.29,48.5,4.63,47.99,0c-3.98,0.16-8.8,2.65-11.66,6 c-2.56,2.96-4.81,7.71-4.2,12.25C36.57,18.59,41.11,15.99,43.87,12.64z M47.89,19.13c-6.52-0.48-12.05,3.64-15.13,3.64 c-3.13,0-7.96-3.54-13.07-3.44c-6.72,0.1-12.91,3.91-16.38,9.93c-6.98,12.11-1.78,30.06,5.02,39.89c3.33,4.8,7.29,10.22,12.5,10.02 c5.01-0.2,6.91-3.24,12.97-3.24c6.06,0,7.77,3.24,13.07,3.15c5.4-0.1,8.82-4.9,12.12-9.73c3.82-5.57,5.39-10.97,5.48-11.26 c-0.12-0.05-10.52-4.04-10.62-16.02c-0.09-10.02,8.18-14.83,8.55-15.07C57.7,20.08,50.45,19.32,47.89,19.13z"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>
                                

                                {/* Now Copy Of Components Above to give loop effect */}
                            

                                <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features126/v4/70/f8/53/70f85351-a46f-39aa-724d-691d23df6893/d32ee8a7-c03d-4459-82aa-5b7cf6dee412.png/834x472.jpg" />
                                    
                                    <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 387 80"><path d="M117.81,58.88H89.96l-6.68,19.75h-11.8L97.86,5.57h12.25l26.38,73.06h-12L117.81,58.88z M92.85,49.77h22.07l-10.89-32.05 h-0.3L92.85,49.77z M142.71,25.36h10.43v9.06h0.25c1.77-6.23,6.53-9.92,12.81-9.92c1.57,0,2.94,0.25,3.75,0.41v10.18 c-0.86-0.35-2.78-0.61-4.91-0.61c-7.04,0-11.39,4.71-11.39,12.25v31.9h-10.94V25.36z M209.95,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L209.95,43.69L209.95,43.69z M226.11,63.49c0-9.32,7.14-15.04,19.8-15.75l14.58-0.86v-4.1 c0-5.92-4-9.47-10.68-9.47c-6.33,0-10.28,3.04-11.24,7.8h-10.33c0.61-9.62,8.81-16.71,21.97-16.71c12.91,0,21.16,6.83,21.16,17.52 v36.71h-10.48v-8.76h-0.25c-3.09,5.92-9.82,9.67-16.81,9.67C233.4,79.54,226.11,73.06,226.11,63.49z M260.49,58.68v-4.2l-13.11,0.81 c-6.53,0.46-10.23,3.34-10.23,7.9c0,4.66,3.85,7.7,9.72,7.7C254.51,70.88,260.49,65.62,260.49,58.68z M279.06,52 c0-16.81,8.91-27.49,22.28-27.49c7.64,0,13.72,3.85,16.71,9.72h0.2V5.57h10.99v73.06H318.6v-9.06h-0.2 c-3.14,6.07-9.21,9.97-16.96,9.97C287.97,79.54,279.06,68.8,279.06,52z M290.25,52c0,11.24,5.42,18.28,14.08,18.28 c8.4,0,14.02-7.14,14.02-18.28c0-11.04-5.62-18.28-14.02-18.28C295.67,33.72,290.25,40.81,290.25,52z M385.16,62.98 c-1.52,9.72-10.73,16.66-22.93,16.66c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58 v3.75h-37.62v0.66c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.83-3.14,12.3-7.8C374.78,62.98,385.16,62.98,385.16,62.98z M348.2,47.08 h26.63c-0.25-8.15-5.42-13.87-13.06-13.87C354.13,33.21,348.76,39.03,348.2,47.08z M43.87,12.7c2.73-3.42,4.59-8.01,4.1-12.7 c-4,0.2-8.88,2.64-11.71,6.06c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.92,19.14 c-6.52-0.39-12.07,3.7-15.18,3.7c-3.12,0-7.89-3.51-13.04-3.41c-6.71,0.1-12.94,3.89-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83 c3.31,4.87,7.3,10.23,12.55,10.03c4.97-0.19,6.91-3.22,12.95-3.22c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74 c3.8-5.55,5.35-10.91,5.45-11.2c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1 C57.75,20.12,50.45,19.34,47.92,19.14"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>


                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative bg-blue-950'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-[55%] h-[90%] pt-5 hidden lg:inline absolute left-4 object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/e9/fd/65/e9fd6546-8620-4445-24bb-feeb6a25e42c/0c233023-fb62-4129-b375-7b9278bb2d78.png/392x392SC.DN01.jpg?l=en-US" />
                                    <img className='w-[45%] md:w-[44%] lg:w-[55%] h-[80%] sm:h-[85%] md:h-[90%] pt-[23px] md:pt-3 lg:hidden absolute left-[28%] object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/e9/fd/65/e9fd6546-8620-4445-24bb-feeb6a25e42c/0c233023-fb62-4129-b375-7b9278bb2d78.png/196x196SC.DN01.jpg?l=en-US" />
                                    
                                    <svg className='w-[65px] lg:w-[73px] h-[12px] lg:h-[16px] absolute bottom-4 right-1 md:right-2 lg:right-3' fill='white' viewBox="0 0 339 80"><path d="M142.14,78.63V24.86h-0.35l-22.02,53.77h-8.4L89.28,24.86h-0.35v53.77H78.6V5.57h13.11l23.64,58.22h0.41l23.59-58.22h13.16 v73.06H142.14z M209.62,78.63h-10.48v-9.01h-0.25c-2.94,6.38-8.35,9.97-16.5,9.97c-11.59,0-18.68-7.54-18.68-19.75V25.36h10.94 v32.35c0,8.1,3.85,12.45,11.39,12.45c7.9,0,12.66-5.47,12.66-13.77V25.36h10.94L209.62,78.63z M239.59,24.4 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.84 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C219.03,30.93,227.34,24.4,239.59,24.4L239.59,24.4z M269.26,11.34 c-0.07-3.51,2.72-6.41,6.23-6.48s6.41,2.72,6.48,6.23s-2.72,6.41-6.23,6.48c-0.05,0-0.1,0-0.15,0 C272.12,17.59,269.29,14.81,269.26,11.34L269.26,11.34z M270.12,25.36h10.94v53.26h-10.94V25.36z M327.22,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L327.22,43.69z M43.87,12.7c2.96-3.54,4.43-8.09,4.1-12.7c-4.54,0.43-8.74,2.6-11.71,6.06 c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.91,19.14c-6.52-0.39-12.07,3.7-15.18,3.7 c-3.12,0-7.89-3.51-13.04-3.41c-6.83,0.17-13.05,3.95-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83c3.31,4.87,7.3,10.23,12.55,10.03 C25.83,79.03,27.77,76,33.81,76c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74c2.35-3.45,4.18-7.22,5.45-11.2 c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1C57.75,20.12,50.45,19.34,47.91,19.14"></path></svg>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>


                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Video221/v4/b1/cd/c7/b1cdc76a-0d1d-fcc2-7e04-ecd73c11442b/media-ST_KN_0056-8864373-Artwork-iOS-283786.png/834x472.jpg" />
                                    
                                   <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 449 80"><path d="M78.6,78.56V5.5h45.62v9.77H89.94v23.14h31.39v9.57H89.94v30.58H78.6z M131.86,11.22c0-3.49,2.83-6.28,6.33-6.28 c3.49,0,6.33,2.79,6.33,6.28c0,3.44-2.83,6.28-6.33,6.28C134.69,17.5,131.86,14.67,131.86,11.22z M132.72,25.3h10.94v53.26h-10.94 V25.3z M170.68,12.69V25.3h10.12v8.66h-10.12v29.36c0,4.61,2.02,6.68,6.48,6.68c1.11,0,2.89-0.1,3.59-0.25v8.66 c-1.21,0.3-3.64,0.51-6.08,0.51c-10.78,0-14.99-4.1-14.99-14.38V33.96h-7.75V25.3h7.75V12.69H170.68z M189.98,25.3h10.43v8.91h0.25 c2.89-6.08,8.56-9.82,16.76-9.82c11.75,0,18.48,7.49,18.48,19.69v34.48h-10.99V46.21c0-8-3.75-12.4-11.34-12.4 c-7.7,0-12.66,5.47-12.66,13.72v31.04h-10.94V25.3z M291.07,62.91c-1.52,9.72-10.73,16.66-22.93,16.66 c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58v3.75h-37.62v0.66 c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.84-3.14,12.3-7.8H291.07z M254.11,47.02h26.63c-0.25-8.15-5.42-13.87-13.06-13.87 C260.03,33.15,254.66,38.97,254.11,47.02z M319.62,24.34c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4 c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4 c-13.32,0-21.32-6.53-22.33-16.66h10.83c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79 l-8.66-2.02c-10.13-2.33-15.14-7.19-15.14-14.94C299.07,30.87,307.37,24.34,319.62,24.34z M369.08,24.34 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.83 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C348.52,30.87,356.83,24.34,369.08,24.34z M428.07,78.63h-9.97V56.93h-20.66v-9.87h20.66V25.37 h9.97v21.69h20.71v9.87h-20.71V78.63z M43.87,12.64C46.64,9.29,48.5,4.63,47.99,0c-3.98,0.16-8.8,2.65-11.66,6 c-2.56,2.96-4.81,7.71-4.2,12.25C36.57,18.59,41.11,15.99,43.87,12.64z M47.89,19.13c-6.52-0.48-12.05,3.64-15.13,3.64 c-3.13,0-7.96-3.54-13.07-3.44c-6.72,0.1-12.91,3.91-16.38,9.93c-6.98,12.11-1.78,30.06,5.02,39.89c3.33,4.8,7.29,10.22,12.5,10.02 c5.01-0.2,6.91-3.24,12.97-3.24c6.06,0,7.77,3.24,13.07,3.15c5.4-0.1,8.82-4.9,12.12-9.73c3.82-5.57,5.39-10.97,5.48-11.26 c-0.12-0.05-10.52-4.04-10.62-16.02c-0.09-10.02,8.18-14.83,8.55-15.07C57.7,20.08,50.45,19.32,47.89,19.13z"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>

                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features116/v4/1c/d7/e1/1cd7e1f9-5c0d-0f83-0900-f9fcad401c05/71e4f76e-d72a-40f5-ad22-382f85f85065.png/834x472.jpg" />
                                    
                                   <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 387 80"><path d="M117.81,58.88H89.96l-6.68,19.75h-11.8L97.86,5.57h12.25l26.38,73.06h-12L117.81,58.88z M92.85,49.77h22.07l-10.89-32.05 h-0.3L92.85,49.77z M142.71,25.36h10.43v9.06h0.25c1.77-6.23,6.53-9.92,12.81-9.92c1.57,0,2.94,0.25,3.75,0.41v10.18 c-0.86-0.35-2.78-0.61-4.91-0.61c-7.04,0-11.39,4.71-11.39,12.25v31.9h-10.94V25.36z M209.95,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L209.95,43.69L209.95,43.69z M226.11,63.49c0-9.32,7.14-15.04,19.8-15.75l14.58-0.86v-4.1 c0-5.92-4-9.47-10.68-9.47c-6.33,0-10.28,3.04-11.24,7.8h-10.33c0.61-9.62,8.81-16.71,21.97-16.71c12.91,0,21.16,6.83,21.16,17.52 v36.71h-10.48v-8.76h-0.25c-3.09,5.92-9.82,9.67-16.81,9.67C233.4,79.54,226.11,73.06,226.11,63.49z M260.49,58.68v-4.2l-13.11,0.81 c-6.53,0.46-10.23,3.34-10.23,7.9c0,4.66,3.85,7.7,9.72,7.7C254.51,70.88,260.49,65.62,260.49,58.68z M279.06,52 c0-16.81,8.91-27.49,22.28-27.49c7.64,0,13.72,3.85,16.71,9.72h0.2V5.57h10.99v73.06H318.6v-9.06h-0.2 c-3.14,6.07-9.21,9.97-16.96,9.97C287.97,79.54,279.06,68.8,279.06,52z M290.25,52c0,11.24,5.42,18.28,14.08,18.28 c8.4,0,14.02-7.14,14.02-18.28c0-11.04-5.62-18.28-14.02-18.28C295.67,33.72,290.25,40.81,290.25,52z M385.16,62.98 c-1.52,9.72-10.73,16.66-22.93,16.66c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58 v3.75h-37.62v0.66c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.83-3.14,12.3-7.8C374.78,62.98,385.16,62.98,385.16,62.98z M348.2,47.08 h26.63c-0.25-8.15-5.42-13.87-13.06-13.87C354.13,33.21,348.76,39.03,348.2,47.08z M43.87,12.7c2.73-3.42,4.59-8.01,4.1-12.7 c-4,0.2-8.88,2.64-11.71,6.06c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.92,19.14 c-6.52-0.39-12.07,3.7-15.18,3.7c-3.12,0-7.89-3.51-13.04-3.41c-6.71,0.1-12.94,3.89-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83 c3.31,4.87,7.3,10.23,12.55,10.03c4.97-0.19,6.91-3.22,12.95-3.22c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74 c3.8-5.55,5.35-10.91,5.45-11.2c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1 C57.75,20.12,50.45,19.34,47.92,19.14"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>

                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative bg-orange-100'>

                                    <img className='w-[55%] h-[90%] pt-5 hidden lg:inline absolute left-4 object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/d9/e4/d2/d9e4d2c4-471e-1d0b-3afa-06fafa91a5c7/afd93571-31f8-49a5-a094-ea42fec182c9.png/392x392SC.DN01.jpg?l=en-US" />
                                    <img className='w-[45%] md:w-[44%] lg:w-[55%] h-[80%] sm:h-[85%] md:h-[90%] pt-[23px] md:pt-3 lg:hidden absolute left-[28%] object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features221/v4/d9/e4/d2/d9e4d2c4-471e-1d0b-3afa-06fafa91a5c7/afd93571-31f8-49a5-a094-ea42fec182c9.png/262x262SC.DN01.jpg?l=en-US" />
                                    
                                    <svg className='w-[65px] lg:w-[73px] h-[12px] lg:h-[16px] absolute bottom-4 right-1 md:right-2 lg:right-3' fill='black' viewBox="0 0 339 80"><path d="M142.14,78.63V24.86h-0.35l-22.02,53.77h-8.4L89.28,24.86h-0.35v53.77H78.6V5.57h13.11l23.64,58.22h0.41l23.59-58.22h13.16 v73.06H142.14z M209.62,78.63h-10.48v-9.01h-0.25c-2.94,6.38-8.35,9.97-16.5,9.97c-11.59,0-18.68-7.54-18.68-19.75V25.36h10.94 v32.35c0,8.1,3.85,12.45,11.39,12.45c7.9,0,12.66-5.47,12.66-13.77V25.36h10.94L209.62,78.63z M239.59,24.4 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.84 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C219.03,30.93,227.34,24.4,239.59,24.4L239.59,24.4z M269.26,11.34 c-0.07-3.51,2.72-6.41,6.23-6.48s6.41,2.72,6.48,6.23s-2.72,6.41-6.23,6.48c-0.05,0-0.1,0-0.15,0 C272.12,17.59,269.29,14.81,269.26,11.34L269.26,11.34z M270.12,25.36h10.94v53.26h-10.94V25.36z M327.22,43.69 c-1.11-5.67-5.37-10.18-12.5-10.18c-8.46,0-14.02,7.04-14.02,18.48c0,11.7,5.62,18.48,14.13,18.48c6.73,0,11.14-3.7,12.4-9.87h10.53 c-1.17,11.34-10.23,19.04-23.04,19.04c-15.24,0-25.16-10.43-25.16-27.64c0-16.91,9.92-27.59,25.06-27.59 c13.72,0,22.07,8.81,23.04,19.29L327.22,43.69z M43.87,12.7c2.96-3.54,4.43-8.09,4.1-12.7c-4.54,0.43-8.74,2.6-11.71,6.06 c-2.54,2.93-4.78,7.71-4.2,12.21C36.55,18.66,41.04,16.02,43.87,12.7 M47.91,19.14c-6.52-0.39-12.07,3.7-15.18,3.7 c-3.12,0-7.89-3.51-13.04-3.41c-6.83,0.17-13.05,3.95-16.35,9.93c-7.01,12.08-1.85,30,4.97,39.83c3.31,4.87,7.3,10.23,12.55,10.03 C25.83,79.03,27.77,76,33.81,76c6.03,0,7.78,3.22,13.04,3.12c5.45-0.1,8.86-4.87,12.17-9.74c2.35-3.45,4.18-7.22,5.45-11.2 c-0.1-0.1-10.51-4.09-10.61-16.07c-0.1-10.03,8.18-14.8,8.56-15.1C57.75,20.12,50.45,19.34,47.91,19.14"></path></svg>

                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>

                            <div className=' h-full min-w-[236px] sm:min-w-[256px] md:min-w-[276px] lg:min-w-[296px] xl:min-w-[316px] 2xl:min-w-[346px] mr-[6px] ml-[6px] box' >
                                
                                <div className='w-full h-[81%] lg:h-full relative'>

                                    <div className="absolute inset-0 transition-all ease-in-out" style={{ background: 'linear-gradient(0deg, rgba(29, 29, 31, 0.5) 0%, rgba(0, 0, 0, 0) 50%)' }}></div>
                                    
                                    <img className='w-full h-full object-cover' src="https://is1-ssl.mzstatic.com/image/thumb/Features116/v4/03/e7/9c/03e79ced-818d-3cf2-995d-b8a86fa70492/118e58b2-3c86-4012-b8ef-9ba05e3f338a.png/572x322.jpg" />
                                    
                                    <svg className='w-[68px] lg:w-[73px] h-[12.5px] lg:h-[16px] absolute bottom-4 right-3' fill='white' viewBox="0 0 449 80"><path d="M78.6,78.56V5.5h45.62v9.77H89.94v23.14h31.39v9.57H89.94v30.58H78.6z M131.86,11.22c0-3.49,2.83-6.28,6.33-6.28 c3.49,0,6.33,2.79,6.33,6.28c0,3.44-2.83,6.28-6.33,6.28C134.69,17.5,131.86,14.67,131.86,11.22z M132.72,25.3h10.94v53.26h-10.94 V25.3z M170.68,12.69V25.3h10.12v8.66h-10.12v29.36c0,4.61,2.02,6.68,6.48,6.68c1.11,0,2.89-0.1,3.59-0.25v8.66 c-1.21,0.3-3.64,0.51-6.08,0.51c-10.78,0-14.99-4.1-14.99-14.38V33.96h-7.75V25.3h7.75V12.69H170.68z M189.98,25.3h10.43v8.91h0.25 c2.89-6.08,8.56-9.82,16.76-9.82c11.75,0,18.48,7.49,18.48,19.69v34.48h-10.99V46.21c0-8-3.75-12.4-11.34-12.4 c-7.7,0-12.66,5.47-12.66,13.72v31.04h-10.94V25.3z M291.07,62.91c-1.52,9.72-10.73,16.66-22.93,16.66 c-15.64,0-25.11-10.58-25.11-27.39c0-16.71,9.62-27.85,24.66-27.85c14.78,0,24,10.43,24,26.58v3.75h-37.62v0.66 c0,9.26,5.57,15.39,14.33,15.39c6.23,0,10.84-3.14,12.3-7.8H291.07z M254.11,47.02h26.63c-0.25-8.15-5.42-13.87-13.06-13.87 C260.03,33.15,254.66,38.97,254.11,47.02z M319.62,24.34c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4 c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4 c-13.32,0-21.32-6.53-22.33-16.66h10.83c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79 l-8.66-2.02c-10.13-2.33-15.14-7.19-15.14-14.94C299.07,30.87,307.37,24.34,319.62,24.34z M369.08,24.34 c12.4,0,20.45,6.89,20.81,16.71h-10.28c-0.61-5.16-4.4-8.4-10.53-8.4c-5.92,0-9.87,2.89-9.87,7.14c0,3.34,2.43,5.62,8.2,6.94 l8.66,1.97c10.89,2.53,15.04,6.78,15.04,14.48c0,9.72-9.06,16.4-21.62,16.4c-13.32,0-21.32-6.53-22.33-16.66h10.83 c1.01,5.57,4.81,8.3,11.49,8.3c6.58,0,10.63-2.73,10.63-7.09c0-3.44-1.97-5.47-7.8-6.79l-8.66-2.02 c-10.13-2.33-15.14-7.19-15.14-14.94C348.52,30.87,356.83,24.34,369.08,24.34z M428.07,78.63h-9.97V56.93h-20.66v-9.87h20.66V25.37 h9.97v21.69h20.71v9.87h-20.71V78.63z M43.87,12.64C46.64,9.29,48.5,4.63,47.99,0c-3.98,0.16-8.8,2.65-11.66,6 c-2.56,2.96-4.81,7.71-4.2,12.25C36.57,18.59,41.11,15.99,43.87,12.64z M47.89,19.13c-6.52-0.48-12.05,3.64-15.13,3.64 c-3.13,0-7.96-3.54-13.07-3.44c-6.72,0.1-12.91,3.91-16.38,9.93c-6.98,12.11-1.78,30.06,5.02,39.89c3.33,4.8,7.29,10.22,12.5,10.02 c5.01-0.2,6.91-3.24,12.97-3.24c6.06,0,7.77,3.24,13.07,3.15c5.4-0.1,8.82-4.9,12.12-9.73c3.82-5.57,5.39-10.97,5.48-11.26 c-0.12-0.05-10.52-4.04-10.62-16.02c-0.09-10.02,8.18-14.83,8.55-15.07C57.7,20.08,50.45,19.32,47.89,19.13z"></path></svg>
                                    <div className='w-full  absolute bottom-3.5 left-3 pt-[12px] hidden lg:inline text-white  text-[14px] font-semibold'>
                                        Hello My Name is Het Patel 
                                    </div>
                                    
                                    <div className="absolute inset-0 transition-all ease-in-out opacity-0 z-10 flex justify-center items-center cover" style={{ backgroundColor:'rgba(0,0,0,0.4'}}>
                                        <div className='h-auto w-auto px-[15px] py-[7px] border rounded-full transition-all ease-in-out translate-y-[25px] bg-white btn'>shop now</div>
                                    </div>

                                </div>

                                <div className='w-full pt-[10px] lg:hidden text-[rgb(29, 29, 31)] text-[14px] font-semibold '>
                                        Hello My Name is Het Patel 
                                </div>

                            </div>
            
                </div>
            </div>
            <div className='w-[22px] h-[22px] border rounded-full border-transparent bg-[#D2D2D7] absolute right-2 bottom-[-30.5px]' id='toggleButton' onClick={togglePlayPause}>
                        {
                            stop?pauseSVG:playSVG 
                        }                        
                </div>
                    
        </div>

</>
    );
}

export default InfinitySlider;
